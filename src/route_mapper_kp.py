...
def add_kilopost_columns(df: pd.DataFrame) -> pd.DataFrame:
    """
    df に以下を追加して返す：
      - d_km : 直前点からの区間距離[km]（先頭行は0）
      - kp_km: 先頭行を0として累積距離[km]
    ※ 連続性（flag=0/1の区切り）は考慮せず、単純に行間ΔLを累積。
      区切りでリセットしたい場合は後述の拡張を適用。
    """

    lat_prev = df["lat"].shift(1).to_numpy()
    lon_prev = df["lon"].shift(1).to_numpy()
    lat_curr = df["lat"].to_numpy()
    lon_curr = df["lon"].to_numpy()

    d = haversine_km(lat_prev, lon_prev, lat_curr, lon_curr)
    d = np.where(np.isnan(d), 0.0, d)

    df = df.copy()
    df["d_km"] = d
    df["kp_km"] = df["d_km"].cumsum()
    return df
