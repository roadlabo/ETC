...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        row = 0
        self._add_form_row(form_grid, row, "生データフォルダ", self._wrap(in_row), "ETC2.0プローブデータの生データの内、様式1-2出力の「OUT1-2」フォルダを指定  この中には data.csv が圧縮されている複数のZIPファイルが格納されているはずです。")
        row += 1
