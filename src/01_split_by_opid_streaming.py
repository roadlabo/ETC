...
def run_split(config: SplitConfig, progress_cb: ProgressCB = None, cancel_flag=None) -> None:
    ...
    try:
        ...
        ended = time.time()
        elapsed_str = _format_hms(ended - started)
        analysis_datetime = time.strftime("%Y/%m/%d %H:%M:%S")

        log.lines.append("")
        log.lines.append("========================================")
        log.lines.append("【第1スクリーニング 実行結果サマリー】")
        log.lines.append("========================================")
        log.lines.append(f"解析日時              : {analysis_datetime}")
        log.lines.append(f"解析時間              : {elapsed_str}")
        log.lines.append("")
        log.lines.append("■ 入力条件")
        log.lines.append(f"  入力フォルダ        : {config.input_dir}")
        log.lines.append(f"  出力フォルダ        : {config.output_dir}")
        log.lines.append(f"  TERM                : {config.term_name}")
        log.lines.append(f"  2次メッシュコード  : {', '.join(config.zip_digit_keys)}")
        log.lines.append("")
        log.lines.append("■ 解析結果")
        log.lines.append(f"  検索ZIP数           : {total_zips}")
        log.lines.append(f"  処理済ZIP数         : {processed}")
        log.lines.append(f"  運行ID総数          : {out_count}")
        log.lines.append(f"  累積行数            : {total_rows}")
        log.lines.append("")
        log.lines.append("========================================")
        log.lines.append(f"ステータス            : {status}")
        log.lines.append("========================================")
        log.lines.append("")
