...
### 列対応（31の最新ヘッダ順）
ビューア内の列番号は 31 の最新 CSV ヘッダ順に合わせており、以下の列を参照する：
- 距離/時間/速度: `計測距離(m)` / `所要時間(s)` / `交差点通過速度(km/h)`
- 交通量/可否: `通過カウント` / `速度算出可否` / `速度算出不可理由`
- 時間帯ヒストグラム: `計測開始_GPS時刻(補間)`（優先）/ `最近接線分_前点_GPS時刻`（フォールバック）
## 出力
