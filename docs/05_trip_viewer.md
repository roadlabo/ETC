...
## これは何をするツール？
CSV に入っている走行軌跡（緯度・経度の点列）を地図に描き、同時に速度の推移もグラフで確認できる「第1・2スクリーニング用トリップビューアー」です。
...
## 起動時の挙動
- フォルダ引数あり: 指定フォルダを初期表示し、CSV 一覧を自動で読み込みます。
- フォルダ引数なし: 起動直後にフォルダ選択ダイアログは出さず、`DIR: 未選択` の空画面で立ち上がります。左上の「フォルダ選択…」から手動指定してください。
...
```bash
python src/05_trip_viewer.py --nogui --folder "D:\...\1st_output"
```

CSV ファイルを直接指定して実行（引数の解釈は実装に依存）
```bash
python src/05_trip_viewer.py --nogui "D:\...\trip.csv"
```
