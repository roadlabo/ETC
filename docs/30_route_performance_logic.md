...
30 を一度実行すると、ルートごとのフォルダに `*_viewer.json`、`*_performance.csv`、`*_daily_hourly_performance.csv`、`*_performance.xlsx` が保存されます。

`*_daily_hourly_performance.csv` と Excel の `daily_hourly` シートには、`YYYYMMDD × 1時間 × 方向 × バケツ` の平均速度、実トリップ数、拡大後交通量が残ります。

その後は生トリップを再解析しなくても、`bat/30_route_performance_viewer.bat` を起動して既存の `30_route_performance` フォルダを選ぶだけで、保存済み JSON からビューアを再生成できます。ビューアでは日付ボタンと時間ボタンを選び、特定の日の特定の1時間だけを表示できます。
