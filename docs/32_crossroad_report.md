...
## 目的（何をする）
`31_crossroad_trip_performance.py` が出力した交差点性能 CSV を集計し、GUIで確認しながらExcelレポートを出力する。
現場確認用のレポート出力・整形ツール。
※31の仕様により「通過は全件出力・所要時間は未算出(空欄)が混在」するため、交通量と遅れ（所要時間）を分離して扱う。
## 位置づけ（分析フロー上のどこ）
- **レポート出力**フェーズ（交差点性能）。
- 性能 CSV を使ったレポート作成・確認を担う。
## 入力
...
`32_crossroad_report.py` の `BATCH_JOBS` に 3ファイル1組を複数記述して実行する。
...
31の出力には以下の列がある：
- `所要時間算出可否`（1/0）
- `所要時間算出不可理由`（OK / OUT_OF_RANGE / TIME_MISSING / NO_SEGMENT）

レポート出力は「交通量＝通過判定OKの行数」「遅れ集計＝所要時間算出可否=1のみ」で表示・集計する。

### 列対応（31の最新ヘッダ）
レポート出力は列名ベースで 31 の最新 CSV ヘッダを参照し、以下の列を使う：
...
- バッチ例: `python 32_crossroad_report.py`（`BATCH_JOBS` が埋まっていれば自動で一括実行）
- GUI例: `BATCH_JOBS=[]` にして `python 32_crossroad_report.py` → GUI 起動 → ダイアログで3ファイル選択
- カレンダーやリストで日付/枝を選択し、遅れ分布・通過件数などを確認。エクスポートボタンで Excel/画像を保存。
## 判定ロジック（重要なものだけ）
- pandas で CSV を読み込み、枝番号・方向・交通量（通過判定OK行数）・遅れ（所要時間算出可否）を基にグラフを生成。
- 交通量（通過台数）は通過判定OKの行数で集計する（所要時間欠損でも台数に含める）。
- 平均遅れ時間・遅れヒストグラムは `所要時間算出可否=1` の行のみを対象とする（所要時間空欄は遅れ集計から除外）。
...
- できないこと: 新たな性能指標の計算、FLAG/距離閾値の変更、ブラウザ経由の共有。レポートで見える値は 31 の出力に依存し、再計算は行わない。
  - 所要時間未算出（空欄）レコードは、交通量には含めるが、遅れ統計には含めない（仕様）。
## よくあるミス
