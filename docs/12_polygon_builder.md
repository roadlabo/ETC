...
## 実行方法
- `bat/12_polygon_builder.bat` をEdgeまたはChromeで開き、[プロジェクトを選択]でプロジェクトフォルダを指定します。ブラウザからフォルダへのアクセス許可を求められたら許可してください。
...
- ブラウザのフォルダアクセス権限を拒否すると保存できません。保存成功時は画面に保存先を表示します。
## 関連スクリプト
