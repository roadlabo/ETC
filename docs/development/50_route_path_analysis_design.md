...
横断改修後、50を `src/unreleased/50_Path_Analysis.py` から **`src/50_Path_Analysis.py`** へ移動した。BAT・CLIテスト・オフライン地図テスト・操作説明を新しい配置へ更新し、共通モジュールの検索パスもsrc直下に合わせた。以下の実装前調査・最終報告にあるunreleasedの記載は移動前の調査履歴。交差点回帰テストの比較対象は移動前コミット `6f24a3b9` に固定している。
...
- エリア作成は14_area_builder。内部ゾーンは12_polygon_builderで作成し、`12_ゾーニングデータ`に保存する。`14_エリアデータ/14_area.geojson` の analysis_area を採用する。
...
11. **OD・Excel・地図**：50_経路分析.xlsx（トリップ分類、分類別集計、Gate ODの方向別件数降順・通過交通内/全交通内割合、メッシュの件数・割合・原点・セルサイズ）、50_report.html、50_map.html。表示群は全交通/各分類/各Gate OD。割合はExcelで表示する。各トリップを1回読み、各群へ訪問セル集合を加算する。対象路線・Gate・analysis_areaも地図に表示する。
