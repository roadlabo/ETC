...
2. **HTML/JS層 (`11_crossroad_sampler.html`)**
   ...
   - JPG保存要求の作成（環境に応じてQt側キャプチャへフォールバック）
...
背景タイルは地理院淡色地図を利用します。ネット接続時はオンラインタイルを優先し、取得できない場合は同梱のオフラインタイルを利用します。
...
- 交差点中心を画面中央に置き、保存用ズーム16へ揃える
- HTML側で画像化できる場合はJPEG DataURL化してQtへ送信
- HTML側で画像化できない場合はQt側で表示中のWeb画面をJPG保存
...
5. Base64 JPEGを書き込み、またはQt側の画面キャプチャでJPGを書き込み
6. 保存成功シグナル発行
...
3. **画像は保存前に縮尺を統一**
   - 保存時は交差点中心を画面中央に置き、ズーム16でキャプチャします。地図を引いた状態で作業していても、保存JPGの縮尺が交差点ごとに揃います。
