...
## 目的（何をする）
第2スクリーニング済みの様式1-2 CSV と交差点定義を入力し、交差点通過性能（流入方向・流出方向・交差点通過速度など）を算出し、交差点ごとに性能 CSV を出力する。
...
※一般的な交差点評価は「遅れ（delay）」が中心だが、本スクリプトでは可視化・比較のために影響区間の平均速度として扱う。
## 位置づけ（分析フロー上のどこ）
- **性能評価**フェーズ（交差点）。
...
### 交通量と速度を分離する（重要）
本スクリプトは「交差点を通ったか（交通量）」と「速度を算出できるか（品質）」を分離して出力する。
- 交通量（通過台数）: **通過判定OKの行を必ず出力**し、`通過カウント`（常に1）を合計して集計する。
- 速度集計: `速度算出可否=1` の行のみを対象とする（`交差点通過速度(km/h)` が空欄の行は速度集計から除外）。

追加列の意味：
- `通過カウント`：通過判定OKのレコードは常に 1（交通量集計用）
- `速度算出可否`：1=算出可、0=算出不可
- `速度算出不可理由`：
  - `OK`：算出可
...
※右左折で実走行距離が僅かに伸びても距離は固定のため、「右左折のペナルティ（減速・待ち）」が速度低下として出やすい。

## 判定ロジック（重要なものだけ）
- TRIP_NO/FLAG に基づきトリップ単位で処理し、交差点中心に最も近い**線分（2点間）**を求める。
- その線分上の最近接点を「算出中心」とし、前後距離（例：-200m/+20m）の境界通過時刻を**道なり距離で線形補間**して所要時間を得る。
- 流入/流出枝番は、中心に最も近い線分の
  - 前点（上流側）を流入方向参照
  - 後点（下流側）を流出方向参照
...
## 速度算出不可条件（交通量からは除外しない）
次の場合でも「通過レコード」は出力する（交通量に含める）が、速度は算出不可として空欄になる。
- GPS時刻が欠損して補間できない → `速度算出可否=0`, `速度算出不可理由=TIME_MISSING`
- 計測区間（前後距離）がトリップの範囲外に出る → `速度算出可否=0`, `速度算出不可理由=OUT_OF_RANGE`
※速度算出の OK/NG 件数は標準出力に表示する。
...
- 2026-01-06: 通過レコードは必ず出力し交通量に含める。速度算出は可否と理由を列で分離（速度欠損でも除外しない）。
