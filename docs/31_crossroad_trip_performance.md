...
### 7.1 角度算出用サンプル距離
- 流入（IN）: `center-20m -> center-5m` の進行方位を求め、180°反転して中心→枝方向に整合。
- 流出（OUT）: `center+5m -> center+20m` の進行方位を求め、そのまま中心→枝方向に利用。

### 7.2 枝番決定
- 交差点定義 CSV の各枝方向角との角度差最小枝を候補化。
- `BRANCH_MAX_ANGLE_DIFF_DEG = 35.0` を超える場合、該当側枝番は未確定（空欄）になります。
- 判定結果は `流入枝番` / `流出枝番`、差分は `流入角度差(deg)` / `流出角度差(deg)` に出力されます。
