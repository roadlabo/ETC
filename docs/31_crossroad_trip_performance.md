...
- 枝判定角度（IN/OUT）の算出に使う2点は以下の通り。
  - IN：中心点の手前 (center-30m → center-5m) の2点で進行方向を推定し、180°反転して center→branch に合わせる。
  - OUT：中心点の先 (center+10m → center+60m) の2点で進行方向を推定し、そのまま center→branch に合わせる。
- カーブ交差点等で IN が 35° を超えて未確定になりやすいため、IN のサンプル距離を短縮して安定化している。
