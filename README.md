...
- `31_crossroad_dircount.py`（deprecated）：旧版の交差点方向別カウント集計（機能は 70 に包含、設計経緯の保存用）。
...
### 70_crossroad_trip_performance.py（現行・正）
交差点を通過したトリップを抽出し、以下の情報を 1 トリップ 1 行で出力する。

- 流入枝番 / 流出枝番
- 交差点前後の道なり距離・所要時間・通過速度
- 前後5点分の経度・緯度・GPS時刻（検証・可視化用）

本スクリプトは **「抽出」を目的**としており、集計・可視化・解釈は行わない。
出力CSVは、人手による可視化・説明・検証の素材として使用する。
...
### 31_crossroad_dircount.py（deprecated）
過去に使用されていた軽量な交差点通過抽出スクリプト。現在は 70_crossroad_trip_performance.py に機能が包含されているため、新規解析では使用しない。

設計経緯の保存および参考用としてのみ残している。
...
- 季節別 9 パターンに対応：
  ...
  8. 秋（森の芸術祭）
  9. 冬（平常）
- 2025 時点では、**様式1-3 ZIP を直接読まない OD リスト方式** が推奨です。`src/40_trip_od_screening.py` で「様式1-3参照ODリスト」を作成し、`src/OD_extractor.py`（新方式）でゾーン集計します。旧 ZIP 直接参照版は `src/OD_extractor_legacy.py` として残しています。
...
## 付録：設計思想
- 本プロジェクトにおけるスクリプトの役割は「データ抽出」までとし、可視化・集計・解釈は人手で行うことを前提とする。
