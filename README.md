...
## スクリプト抜粋
- 71_Path_Analysis.py：指定断面（交差点中心）を通過したトリップの「流入経路(in)」「流出経路(out)」を10mメッシュで集計し、ヒートマップHTMLを出力する。A/B判定は交通の進行方向ではなく「交差点中心にどちら側から到達したか（流入側）」で判定する。交差点ファイル2行目をA方向、3行目をB方向とし、dir_degは外側→中心の方位角として扱う。出力は (stem)_heatmap_A方向交通.html と (stem)_heatmap_B方向交通.html（左右に流入/流出を並べる）である。
